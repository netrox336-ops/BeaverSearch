...
Need $market 'PositiveTtl\s*=\s*TimeSpan\.FromMinutes\(30\)' 'Steam Market positive cache is not 30 minutes.'
Need $market '_requestGate\s*=\s*new\(1, 1\)' 'Steam Market fallback requests are not serialized.'
Need $market 'TimeSpan\.FromSeconds\(6\)' 'Steam Market fallback pacing is not 6 seconds.'
...
Need $bulk 'MaxSteamFallbackNames\s*=\s*4' 'Steam Market fallback must stay capped at 4 names per valuation.'
...
Write-Host '- pricing: CS2/Dota bulk feeds first; Steam Market fallback max 4 / 6s pacing: checked'
