...
Need $market 'TimeSpan\.FromMilliseconds\(850\)' 'Steam Market fallback pacing marker missing.'
...
Need $valuation 'TemporaryFailure' 'Temporary per-game valuation marker missing.'
Need $models 'HasTemporaryFailures' 'Player partial-valuation state marker missing.'

Need $inventory 'PageSize\s*=\s*1000' 'Steam inventory page size must be 1000.'
Need $inventory '_inventoryRequestGate\s*=\s*new\(1, 1\)' 'Steam inventory requests must be globally serialized.'
Need $inventory 'NormalSpacing\s*=\s*TimeSpan\.FromSeconds\(3\)' 'Steam inventory normal pacing marker missing.'
Need $inventory 'ThrottledSpacing\s*=\s*TimeSpan\.FromSeconds\(6\)' 'Steam inventory throttled pacing marker missing.'
Need $inventory 'GetLegacyInventoryAsync' 'Legacy Steam profile inventory fallback marker missing.'
Need $inventory 'inventory/json/' 'Legacy Steam profile inventory endpoint marker missing.'
Need $inventory 'PrimeProfileInventoryAsync' 'Steam profile inventory cookie-prime fallback marker missing.'
...
Need $cache 'PriceEngineVersion\s*\{\s*get;\s*set;\s*\}\s*=\s*5' 'PriceEngineVersion 5 marker missing.'
Need $local 'CurrentPriceEngineVersion\s*=\s*5' 'LocalStore price-engine v5 migration marker missing.'
...
Need $vm 'MaxScheduledScansPerBatch\s*=\s*40' 'Monitoring per-batch player scan budget is not 40.'
...
Write-Host '- monitoring: 10 servers / max 40 new scans / full batch completion, no overlap: checked'
Write-Host '- source HTML/JSON parsing off WPF dispatcher: checked'
Write-Host '- Steam inventory: modern + legacy profile JSON fallback / one request lane / adaptive cooldown: checked'
Write-Host '- Steam inventory: anonymous session + profile prime + null-body diagnostics: checked'
Write-Host '- pricing: CS2 SkinCash + Dota public RUB feed + Skinport Brotli + Steam Market fallback: checked'
Write-Host '- partial valid valuations survive another game temporary failure: checked'
Write-Host '- false-zero cache migration v5: checked'
